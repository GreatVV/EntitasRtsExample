...
public class Swordsman : MonoBehaviour, IPlayerInitable, IPointerClickHandler
{
    ...
    {
        Entity = pool.CreateEntity().IsUnit(true).AddPlayer(playerId).IsSword(true).AddHealth(StartHealth).AddRegeneration(RegenerationRate).AddDamage(Damage).AddGameObject(gameObject);
    }
